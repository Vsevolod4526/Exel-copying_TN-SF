...
root=Tk()
root.geometry('700x1000+500+20')

root.title("Копирование данных из отчётов .xls")


f_top = Frame()
b_spravka=Button(f_top,text='Справка о правилах использования программы',width=60,height=1,bg='green')


b1=Button(text='Выбрать директорию с файлами',width=30,height=1)
l1=Label(text='Укажите путь в папку с товарными накладными',font='Arial 16')
l2=Label(text=' с расширением .xlsx',font='Arial 16')
e1=Entry(width=100)

b2=Button(text='Запуск копирования',width=30,height=1)
l2marker=Label(text='данные ТН пока не скопированы ',font='Arial 10')


e1promegTN=Entry(width=100)
b1promeg=Button(text='Выбрать директорию для сохранения промежуточного массива из ТН в EXEL',width=70,height=2)
b2promeg=Button(text='Запуск Сохранения',width=30,height=1)
...
b_2Val=Button(text='Выбрать путь к файлу ',width=40,height=1)
b_3Val=Button(text='Сохранить данные по валюте ',width=40,height=1)
l_2Val=Label(text='Данные из файла с валютой пока не скопированы',font='Arial 10')
b_4Val=Button(text='Интегрировать данные валюты ',width=40,height=1)
...
e21=Entry(width=100)
b21=Button(text='Выбрать директорию с файлами',width=30,height=1)
b22=Button(text='Запуск копирования',width=30,height=1)
l22marker=Label(text='данные CФ пока не скопированы ',font='Arial 10')
e21promegSF=Entry(width=100)
b21promeg=Button(text='Выбрать директорию для сохранения промежуточного массива из CФ в EXEL',width=70,height=2)
b22promeg=Button(text='Запуск Сохранения',width=30,height=1)
...
e5=Entry(width=100)
b5=Button(text='Выбор директории для стокового EXEL',width=30,height=1)
b52=Button(text='Запуск создания стокового EXEL',width=30,height=1)
...
def show_spravka():
    messagebox.showinfo(" Для корректной работы программы важно ", ("1. Чтобы все файлы,которые используются в программы были закрыты(файлы ТН, файлы СФ,файл с валютой) \n"+
...
def zapusk_kopy_SF():
    ...
    for l in range(len(list_of_SF_xl)):
        ...
        print(sheet_xlrd.cell(0,1).value)  # cell( Nомер строки, Nомер колонки)
        print(type(sheet_xlrd.cell(0,1).value))
        ...
        if succes!=1:
            ...
            succes=0
        print(sposok_nomerov_strok_dla_copirovania)
        ...
        print(list_of_kod_tovar_uslug)
        print(list_of_kod_vida_tovara)
        print(list_of_nomer_tamojnoy_declaracii)
        print(list_of_strana)
        ...
        print()
        for irow in spisok_massivov_is_SF:
            print(irow)
    l22marker['bg']="#007dff"
...
def creating_EXEL_and_copying_there():
    ...
    for n in range(len(spisok_massivov_dla_zapici)):
        for m in range(len(spisok_massivov_is_SF)):
            if (spisok_massivov_dla_zapici[n][19]==spisok_massivov_is_SF[m][18] and
                spisok_massivov_dla_zapici[n][22]==spisok_massivov_is_SF[m][21]):
...
def zapusk_kopy():
    ...
    for l in range(len(list_of_tovarni_nakladniyi_xl)):
        ...
        print(spisok_kodov_is_TN)
        print(list_of_prices)
        print(list_of_kol_and_massa)
        ...
        for i in range(len(rows_list_to_find)):
            ...
            if len(list_of_naimenovaniy_tovar[i])<50:
                row_list[3]=spisok_kodov_is_TN[i]+' '+list_of_naimenovaniy_tovar[i]
            else:
                row_list[3]=spisok_kodov_is_TN[i]+' '+list_of_naimenovaniy_tovar[i][:50]
